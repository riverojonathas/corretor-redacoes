...
import { User, ChevronDown, LogOut, Key, HelpCircle, Lightbulb, Bug } from 'lucide-react';
            ...
            <header className="h-16 bg-[#fdfaf2]/80 backdrop-blur-md border-b border-[#eee9df] flex items-center justify-between px-8 sticky top-0 z-40">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <button
                        ...
                        className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold text-xs rounded-lg transition-colors border border-emerald-200/60"
                    ...
                    <button
                        ...
                        className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 font-bold text-xs rounded-lg transition-colors border border-red-200/60"
                        title="Reportar Erro"
                    >
                        <Bug size={13} />
                        <span>Reportar</span>
                        ...
                        {isProfileOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50 animate-in fade-in slide-in-from-top-2">
                                <div className="px-4 py-2 border-b border-gray-100 mb-2 sm:hidden">
                                    <p className="text-xs font-semibold text-dark-gray truncate">{nome || user?.email}</p>
                                    <p className="text-[10px] text-gray-500 uppercase mt-0.5">{cargo || 'Membro'}</p>
                                </div>

                                {/* Feedback no mobile (não tem os botões no header pequeno) */}
                                <button
                                    onClick={() => { setIsProfileOpen(false); openFeedback('sugestao'); }}
                                    className="flex sm:hidden items-center gap-2 px-4 py-2 text-sm text-emerald-700 hover:bg-emerald-50 transition-colors w-full text-left"
                                >
                                    <Lightbulb size={14} /> Sugerir Melhoria
                                </button>
                                <button
                                    onClick={() => { setIsProfileOpen(false); openFeedback('bug'); }}
                                    className="flex sm:hidden items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors w-full text-left"
                                >
